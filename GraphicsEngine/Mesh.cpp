...
namespace gps {
	...
	void Mesh::Draw(gps::Shader shader)
	{
		...
		{
			if (textures.at(i).type.compare("normalTexture") == 0)
				hasNormal = 1;
			
			glActiveTexture(GL_TEXTURE0 + i + nrOfFramebuffers);
			glUniform1i(glGetUniformLocation(shader.shaderProgram, this->textures[i].type.c_str()), i + nrOfFramebuffers);
			glBindTexture(GL_TEXTURE_2D, this->textures[i].id);
		}
		glUniform1i(glGetUniformLocation(shader.shaderProgram, "hasNormal"), hasNormal);
