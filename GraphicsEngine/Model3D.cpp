...
namespace gps {
		...
		for (size_t s = 0; s < shapes.size(); s++) {
			...
			if (a > 0 && materials.size()>0) {
				...
				if (materialId != -1) {
					...
					//metal texture
					std::string metalTexturePath = materials[materialId].metallic_texname;
					if (!metalTexturePath.empty())
					{
						gps::Texture currentTexture;
						currentTexture = LoadTexture(basePath + metalTexturePath, "metalTexture");
						textures.push_back(currentTexture);
					}
				}
			}
	...
	gps::Texture Model3D::LoadTexture(std::string path, std::string type) {
			...
			gps::Texture currentTexture;
			currentTexture.id = ReadTextureFromFile(path.c_str(), type);
	...
	GLuint Model3D::ReadTextureFromFile(const char* file_name, std::string type) {
		int x, y, n;
		...
		if(type.compare("diffuseTexture")==0)
			glTexImage2D(GL_TEXTURE_2D, 0, GL_SRGB, x, y, 0, GL_RGBA, GL_UNSIGNED_BYTE, image_data);
		else if(type.compare("roughnessTexture") == 0 || type.compare("metalTexture") == 0)
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, x, y, 0, GL_RGBA, GL_UNSIGNED_BYTE, image_data);
		else
			glTexImage2D(GL_TEXTURE_2D, 0, GL_RGB, x, y, 0, GL_RGBA, GL_UNSIGNED_BYTE, image_data);
